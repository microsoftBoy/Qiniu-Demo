...
import com.zhangshuai.qiniu_demo.Utils.CollectionUtils;
...
//        }
        ...
        if (childs != null) {
            ...
            for (View child : childs) {
                if (child.getVisibility() == VISIBLE) {
                    ...
                    child.layout(paddingLeft, curTop, paddingLeft + child.getMeasuredWidth(),
                            curTop + child.getMeasuredHeight());
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mVerticalRange = mTopView.getMeasuredHeight() - mContentMarginTop;
        Log.i(TAG, "onSizeChanged: mVerticalRange =" + mVerticalRange);
    }
    ...
    private void sendCancelEvent() {
        // The ScrollChecker will update position and lead to send cancel event when
        // mLastMoveEvent is null.
        // fix #104, #80, #92
        ...
        MotionEvent e = MotionEvent.obtain(last.getDownTime(), last.getEventTime() +
                        ViewConfiguration.getLongPressTimeout(), MotionEvent.ACTION_CANCEL, last
                        .getX(),
    ...
    private MotionEvent createCancel() {
        ...
        final MotionEvent ev = MotionEvent.obtain(time, time, MotionEvent.ACTION_CANCEL, 0,
                mLastMotionY, 0);
    ...
    private ParentState mParentState = ParentState.COLLAPSED;
    ...
    private ParentState getParnetState() {
        ...
        if (mTopView instanceof ScrollView) {


            ScrollView mScrollView = (ScrollView) mTopView;
            View childAt = mScrollView.getChildAt(0);

            int y = childAt.getHeight() - mScrollView.getHeight();
            Log.i(TAG, "getParnetState: y = " + y);
            Log.i(TAG, "getParnetState: scrollY= " + scrollY);
            Log.i(TAG, "getParnetState: mVerticalRange= " + mVerticalRange);
            Log.i(TAG, "getParnetState: mScrollView.getScrollY()= " + mScrollView.getScrollY());
            if (scrollY == 0) {
                return ParentState.EXPANDED;
            } else if (scrollY == mVerticalRange) {
                return ParentState.COLLAPSED;
            } else if (mScrollView.getScrollY() < y) {
                return ParentState.COLLAPSED;
            }
            else {
        ...
        LinearLayout linearLayout =(LinearLayout)mTopView;
        RecyclerView recyclerView = (RecyclerView) linearLayout.getChildAt(0);
        float recyclerViewHeight = getRecyclerViewHeight(recyclerView);
        float scollYDistance = getScollYDistance(recyclerView);
        float y = recyclerViewHeight - recyclerView.getHeight();
        Log.i(TAG, "getParnetState: scollYDistance= " + scollYDistance);
        Log.i(TAG, "getParnetState: y = " + y);


        if (scrollY == 0) {
            Log.i(TAG, "getParnetState: EXPANDED");
            return ParentState.EXPANDED;
        } else if (scrollY == mVerticalRange) {
            Log.i(TAG, "getParnetState: COLLAPSED");
            return ParentState.COLLAPSED;
        }
        else if (scollYDistance < y) {
            Log.i(TAG, "getParnetState: COLLAPSED");
            return ParentState.COLLAPSED;
        }
        else if (scrollY == 0&&scollYDistance !=0) {
            Log.i(TAG, "getParnetState: COLLAPSED");
            return ParentState.COLLAPSED;
        }
        else {
            Log.i(TAG, "getParnetState: SCROLL");
            return ParentState.SCROLL;
        }
    }
...
//        Log.i(TAG, "dispatchTouchEvent: ");
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                int activePointerIndex = MotionEventCompat.findPointerIndex(event,
                        mActivePointerId);
                ...
                Log.i(TAG, "dispatchTouchEvent: mDragging = " + mDragging);
                if (!mDragging) {
                    float dy = y - mFirstMotionY;
                    float dx = x - mFirstMotionX;
                    if (Math.abs(dy) > mTouchSlop) {
                        if (Math.abs(dy) > Math.abs(dx)) {
                            Log.i(TAG, "dispatchTouchEvent: 垂直滑动");
                            Log.i(TAG, "dispatchTouchEvent: dy = " + dy);
                            mDragging = true;
                            mLastMotionY = y;
                            mLastMotionX = x;
                            isHandlar = doScrollAndCallSupper(event, 0);
                            Log.i(TAG, "dispatchTouchEvent: 垂直滑动 isHandlar = " + isHandlar);
                            break;
                        } else {
                            mDragging = false;
                ...
                /*Log.i(TAG, "dispatchTouchEvent: 垂直滑动");
                View childView = ((ScrollView) mTopView).getChildAt(0);
                if (((ScrollView) mTopView).getScrollY() != (childView.getHeight() - (
                (ScrollView) mTopView).getHeight() - 500)) {
                    //滑动到底部
                    //TODO
                ...
                Log.i(TAG, "dispatchTouchEvent: isHandlar= " + isHandlar);
                Log.i(TAG, "dispatchTouchEvent: mParentState= " + mParentState);
                Log.i(TAG, "dispatchTouchEvent: curState= " + curState);
...
//                        MotionEventCompat.findPointerIndex(event, mActivePointerId));
            ...
            case MotionEvent.ACTION_UP:
                ...
                int velocityY = (int) VelocityTrackerCompat.getYVelocity(mVelocityTracker,
                        mActivePointerId);
...
//        event.offsetLocation(mLastMotionX - curX ,0);
        ...
        if (isChanged) {
            Log.i(TAG, "doScrollAndCallSupper: isChanged = " + isChanged);
            scrollBy(0, (int) -dy);
            return true;
        }
        Log.i(TAG, "doScrollAndCallSupper: mDragState = " + mDragState);
        switch (mDragState) {
    ...
    private void onSecondaryPointerUp(MotionEvent ev) {
        ...
        Log.e(TAG, pointerIndex + "pointerIndex" + pointerId + " = id    up" + index + "count ="
                + MotionEventCompat.getPointerCount(ev));
    ...
    private boolean isScrollView(View child) {
        if (child instanceof android.support.v4.view.NestedScrollingChild || child instanceof
                AbsListView || child instanceof ScrollView || child instanceof ViewPager || child
                instanceof WebView || child instanceof RecyclerView) {
    ...
    public void scrollTo(int x, int y) {
        ...
        if (y != getScrollY()) {
            ...
            if (mMaxTopTranslationYRate != 0) {
                ...
                Log.i(TAG, "scrollTo: totalOffset =" + totalOffset);
                Log.i(TAG, "scrollTo: verticalOffset =" + verticalOffset);
...
//                Logger.e( reachChildTop() + "cury" + curY +"velocityY = "+velocityY + "
// startY=" + startY +" finalY= " +finalY);
                if (reachChildTop()) {
                    if (Math.abs(velocityY) >= mMinimumVelocity) {
                        mScroller.abortAnimation();
                        fling((int) velocityY);
...
//            return di.refreshRate;
    ...
    public static class CurrentPagerAdapter extends PagerAdapter {
        ...
        public CharSequence getPageTitle(int position) {
            if (!CollectionUtils.isValid(mTitles)) {
    ...
    ViewPager.OnPageChangeListener mOnPageChangeListener = new ViewPager.OnPageChangeListener() {
        ...
        public void onPageSelected(int position) {
            ...
            if (a instanceof FragmentPagerAdapter) {
                ...
                Log.e(TAG, currentPagerAdapter.getPrimaryItem() + "             onPageSelected" +
                        mScrollableView);
            }
    ...
    private boolean isNotifyLayout;
    ...
    public float getScollYDistance(RecyclerView recyclerView) {
    ...
    public float getRecyclerViewHeight(RecyclerView recyclerView) {
        LinearLayoutManager layoutManager = (LinearLayoutManager) recyclerView.getLayoutManager();
        int position = layoutManager.findFirstVisibleItemPosition();
        View firstVisiableChildView = layoutManager.findViewByPosition(position);
        int itemHeight = firstVisiableChildView.getHeight();
